...
- **Interactive Web Explorer:** Provides a UI for exploring the deobfuscated codebase and its associated graphs, designed for rapid security analysis. Includes a dedicated **Security Analysis View** for interactive exploration of taint flows.
...
- **Structured Security Reporting:** Automatically generates comprehensive security reports in JSON format (via CLI `--security-report` or Web UI export), including API surfaces, DOM findings, and analyzed taint paths.
