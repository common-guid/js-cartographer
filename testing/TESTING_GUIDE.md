...
13. [Quality Evaluation Suite](#13-quality-evaluation-suite)
14. [Rules & Checklist](#14-rules--checklist)
...
## 13. Quality Evaluation Suite

In addition to the functional test suite, JS Cartographer includes a **Quality Evaluation Suite** to measure the effectiveness of the deobfuscation and renaming process.

### How it works
The suite runs the unminifier against a known ground truth (`fixtures/webpack-hello-world/dist/bundle.js`) and performs two checks:

1. **Source Map Recovery Score**: Compares the names generated by the tool against the original names stored in a source map. This yields an objective percentage (e.g., "75% of original names recovered").
2. **Snapshot Diffing**: Compares the current unminified output against a "golden" snapshot. If changes occur (e.g., due to model non-determinism), it shows a colored diff and asks the developer to accept or reject the change.

### Running the suite
```bash
# Optional: generate a fresh source map for the fixture
cd fixtures/webpack-hello-world && npm run build:sourcemap && cd ../..

# Run the quality evaluation (requires OPENAI_API_KEY for rename scoring)
npm run test:quality
```

### When to run this
- When modifying LLM prompts.
- When adding new AST transformation rules (Wakaru rules).
- Before a major release to ensure no degradation in unminification quality.
